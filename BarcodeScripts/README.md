...
```bash
# Barcode decode
time perl ${baseDir}/BarDecodeRGv26.pl \
  --input-1 ${workDir}/${prefix}_step1_R1_trimmed.fastq.gz \
  --input-2 ${workDir}/${prefix}_step1_R2_trimmed.fastq.gz \
  --output-1 ${workDir}/${prefix}_starInput_1.fastq \
  --output-2 ${workDir}/${prefix}_starInput_2.fastq \
  --discards ${tempDir}/${prefix}_bardecode_discards.sam

# Collapse barcodes
python ${baseDir}/CollapseBarcodeTags.py \
  --input ${tempDir}/${prefix}Aligned.sortedByCoord.out.bam \
  --barcodes1 ${baseDir}/bc1_v26.txt \
  --barcodes2 ${baseDir}/bc2_v26.txt \
  --output ${workDir}/${prefix}_collapsed.bam \
  --perl-diff
```
